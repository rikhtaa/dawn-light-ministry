...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/ministries/education">): Promise<Metadata> {
  const { locale: rawLocale } = await params;
  const locale = isLocale(rawLocale) ? rawLocale : "en";
  const strings = getMinistryPagesContent(locale).education;

  return {
    title: `${strings.title} — Dawn of Light Ministry`,
    description: strings.standfirst,
    alternates: {
      canonical: `/${locale}/ministries/education`,
      languages: { en: "/en/ministries/education", ur: "/ur/ministries/education" },
    },
  };
}

/**
 * Dawn of Light - Ministry Pages.dc.html "04 — Education" (`/ministries/education`,
 * body only). Two genuine differences from Church/Publishing/Children's
 * Education: the at-a-glance strip is 3 facts, not 4, and the navy detail
 * band is a stated quote + body paragraph rather than a ruled-row list —
 * reproduced as drawn, not forced into the other pages' shape.
 */
export default async function EducationPage({
  params,
}: PageProps<"/[locale]/ministries/education">) {
  ...
  const d = strings.education;
  const s = strings.shared;

  const facts = [
    d.facts.approach,
    d.facts.cities,
    d.facts.programmesAndReach,
  ];

  const activities = [
    d.body.activities.education,
    d.body.activities.teaching,
    d.body.activities.lectures,
    d.body.activities.material,
  ];

  const siblingKeys = ["church", "seminary", "publishing", "childrensEducation"] as const;
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <div className="border-b border-border bg-surface">
        <Container>
          <FactTable layout="row" columns={3} facts={facts} isUrdu={isUrdu} />
        </Container>
      </div>
      ...
      <section className="bg-surface py-16 lg:py-24">
        <Container className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_380px] lg:gap-16">
          <Reveal className="min-w-0">
            ...
            <ArticleBody blocks={educationBodyBlocks} content={d.body.blocks} isUrdu={isUrdu} className="mt-5" />
          ...
          <Reveal className="flex min-w-0 flex-col gap-3.5">
            <Card topRule="navy" tone="surface-warm">
              ...
              <div className="mt-3.5 flex flex-col gap-2.5">
                ...
                <Button href={path("/support")} variant="secondary" isUrdu={isUrdu}>
                  {d.rail.secondaryCta}
                </Button>
              </div>
            </Card>
            ...
            <Card tone="surface">
              ...
              <div className="mt-2.5 flex flex-col items-start gap-2">
                <Button href={path("/ministries/childrens-education")} variant="tertiary" showArrow isUrdu={isUrdu}>
                  {d.rail.related.childrensEducation}
                </Button>
                <Button href={`${path("/about")}#mission-vision`} variant="tertiary" showArrow isUrdu={isUrdu}>
                  {d.rail.related.missionAndVision}
                </Button>
      ...
      <NavyBand>
        <Reveal className="grid grid-cols-1 gap-8 lg:grid-cols-[280px_1fr] lg:gap-14">
          ...
          <div>
            <p className={cn("text-h3 max-w-[34ch] font-normal text-dark-heading", isUrdu && "font-urdu-display")}>
              {d.detailBand.quote}
            </p>
            <p className={cn("text-body measure mt-4 text-dark-body", isUrdu && "font-urdu-body")}>
              {d.detailBand.quoteBody}
            </p>
          </div>
        </Reveal>
      </NavyBand>
